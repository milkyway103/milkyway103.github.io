...
-	`import React from 'react'`
	...
	-	리액트 프로젝트를 만들 때 `node_modules`라는 디렉터리도 함께 생성되는데, 프로젝트 생성 과정에서 `node_modulres` 디렉터리에 `react` 모듈이 설치된다. 그리고 `import` 구문을 통해 리액트를 불러와서 사용할 수 있게 된다.
...
웹팩을 사용하면 SVG 파일과 CSS 파일도 불러와서 사용할 수 있다. 이렇게 파일들을 불러오는 것은 **웹팩의 로더(loader)** 라는 기능이 담당한다. 로더는 여러 가지 종류가 있다. - css-loader: CSS 파일을 불러올 수 있게 해 준다. - file-loader: 웹 폰트나 미디어 파일 등을 불러올 수 있게 해 준다. - babel-loader: 자바스크립트 파일들을 불러오면서 최신 자바스크립트 문법으로 작성된 코드를 바벨이라는 도구를 사용하여 ES5 문법으로 변환해 준다.
...
`let`과 `const`는 scope가 함수ㅜ 단위가 아닌 블록 단위이므로, if문 내부에서 선언한 a값은 if문 밖의 a값을 변경하지 않는다.
